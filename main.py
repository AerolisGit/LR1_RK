...
def generate_map(width, height, wall_probability=0.1):
    ...
    for i in range(height):
        ...
        for j in range(width):
            # Используем детерминированный паттерн вместо случайного выбора
...
def generate_enemies(width, height, num_enemies):
    enemies = []
    # Последовательность врагов циклически повторяется
...
while True:
    ...
    for enemy in enemies[:]:  # использование enemies[:] для итерации по копии списка
